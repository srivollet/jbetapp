...
public abstract class FlashresultatConnector {
	...
	public static List<Match> run() throws BetappException {
		...
		try {
			...
			Calendar calendar = DateUtil.getCalendar_ddMM(driver.findElement(
					By.className("today")).getText());

			WebElement resultats = driver.findElement(By
			...
			for (Element soccer : soccers) {
				Elements elements = soccer.getElementsByTag("tbody").first()
						.getElementsByTag("tr");
	...
	private static Match getMatch(Element element, Calendar calendar) {
		...
		equipe1.setNom(getString(element.getElementsByClass("team-home")
				.first().text()));
		match.setEquipe1(equipe1);

		Equipe equipe2 = new Equipe();
		equipe2.setNom(getString(element.getElementsByClass("team-away")
				.first().text()));
		match.setEquipe2(equipe2);

		String s = getString(
				element.getElementsByClass("score").first().text(),
		...
		match.setCote1(getFloat(element.getElementsByClass("cell_oa").first()
				.text()));
		match.setCoteN(getFloat(element.getElementsByClass("cell_ob").first()
				.text()));
		match.setCote2(getFloat(element.getElementsByClass("cell_oc").first()
				.text()));
